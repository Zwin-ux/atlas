...
import { createGazetteer, type GazetteerPlace } from "@atlas/core/atlas";

import type { AtlasIndex } from "../src/atlasIndex.js";
import {
  ATLAS_PLACE_CANDIDATE_MAX,
  createAtlasPlaceSearch,
  validateAtlasPlaceQuery,
} from "../src/atlasPlaceSearch.js";
...
test("search returns bounded public candidates without coordinates", () => {
  const search = createAtlasPlaceSearch(index);
  const result = search.search("Riverside", 99);
...
test("resolution preserves ambiguity instead of choosing a Springfield", () => {
  const result = createAtlasPlaceSearch(index).resolve("Springfield");
  assert.equal(result.status, "ambiguous");
  if (result.status !== "ambiguous") return;
  assert.equal(result.candidates.length, 2);
  assert.deepEqual(new Set(result.candidates.map((candidate) => candidate.state)), new Set(["il", "mo"]));
});

test("resolution returns an honest unresolved result for an unknown query", () => {
  const result = createAtlasPlaceSearch(index).resolve("Atlantis-by-the-Pacific");
  assert.equal(result.status, "unresolved");
  if (result.status !== "unresolved") return;
  assert.ok(result.candidates.length <= ATLAS_PLACE_CANDIDATE_MAX);
});
