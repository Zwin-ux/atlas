...
| Live ChatGPT end-to-end evidence environment | PUBLIC CHROME AND REAL CODEX IN-APP VERIFIED / CHATGPT, MODEL GATED | Exact private-remote candidate `97792d84` is live at `https://atlas-webmcp-production.up.railway.app/explore` through isolated deployment `99367264`. HTTPS readiness, no-login, exact-five metadata, Chrome 152 Site Tools execution, visible atomic writes, Springfield safety, human marker-to-agent state handoff, fallback, exact 390x844 proof, and reduced motion pass. Prior real Codex in-app proof covers the unchanged exact-five controller architecture; the real ChatGPT transcript and exact `grok-4.6` three-run/90% lane remain separately gated. |
...
| Clean-clone and public-release safety audit | PRIVATE REMOTE AND DEPLOYMENT VERIFIED / PUBLICATION OWNER-GATED | Private remote `main` and active Railway deployment `99367264` identify exact presentation candidate `97792d84d66239745011624690769e99ab25838c`, projected from source `da0f4eae8582a345ada3de891422a21b0ac9c4dd`. It remains private and passes the complete no-local clean-clone sequence, 39/39 verifier, public exact-five Chrome proof, release audit, clean status, and a 16-commit Gitleaks scan with zero findings. Public visibility and submission remain separate owner gates. |
