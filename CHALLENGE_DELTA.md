...
| Live ChatGPT end-to-end evidence environment | PUBLIC CHROME VERIFIED / UPDATE OWNER-GATED / CHATGPT, MODEL GATED | Exact candidate `986cf864` remains live at `https://atlas-webmcp-production.up.railway.app/explore`; HTTPS readiness, exact-five metadata, Chrome 152 protocol execution, fallback, and responsive Product Design proof pass. Local candidate `110fd05f` strengthens immediate visible completion, mobile map/target proof, and the one-command acceptance session: it rejects executable URL/path interpolation, copied template toggles, uncontained or oversized evidence, fake screenshot files, duplicate call IDs, and mutation claims without matching post-failure state reads. The private push/deployment, real ChatGPT transcript, and exact `grok-4.6` three-run/90% lane remain separately gated. |
...
| Clean-clone and public-release safety audit | NEW LOCAL CANDIDATE VERIFIED / PRIVATE PUSH, DEPLOYMENT, PUBLICATION OWNER-GATED | Exact sanitized candidate `110fd05f9fb4a78f66954ea3b0c952012b512e9a`, projected from source `abfe898a4793f60b803b2c2ce108ecea75f8cb7f`, passed a no-local clean-clone frozen install, typecheck, build, 37/37 verifier, Chrome smoke with 9/9 official steps and 15 deeper executions, release audit, diff check, and clean-status proof. Private remote `main` and the active isolated Railway deployment deliberately remain on `986cf864927219f5f269a04b21a96f8440988cba` pending owner approval. |
