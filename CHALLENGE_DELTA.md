...
| Live ChatGPT end-to-end evidence environment | PUBLIC CHROME VERIFIED / CHATGPT, MODEL GATED | Exact candidate `986cf864` remains live at `https://atlas-webmcp-production.up.railway.app/explore`; HTTPS readiness, exact-five metadata, Chrome 152 protocol execution, fallback, and responsive Product Design proof pass. The local one-command acceptance session rejects executable URL/path interpolation, copied template toggles, uncontained or oversized evidence, fake screenshot files, and mutation claims without matching post-failure state reads; the real ChatGPT transcript and exact `grok-4.6` three-run/90% lane remain credential-gated. |
