...
| Live ChatGPT end-to-end evidence environment | PUBLIC CHROME VERIFIED / UPDATE OWNER-GATED / CHATGPT, MODEL GATED | Exact candidate `986cf864` remains live at `https://atlas-webmcp-production.up.railway.app/explore`; HTTPS readiness, exact-five metadata, Chrome 152 protocol execution, fallback, and responsive Product Design proof pass. Local candidate `111b8fd` removes unsolicited idle coaching, strengthens immediate visible completion and mobile map/target proof, and hardens the one-command acceptance session against executable URL/path interpolation, copied template toggles, uncontained or oversized evidence, fake screenshot files, duplicate call IDs, and mutation claims without matching post-failure state reads. The private push/deployment, real ChatGPT transcript, and exact `grok-4.6` three-run/90% lane remain separately gated. |
| Judge-path accessibility and desktop/mobile proof | VERIFIED | Shared keyboard/touch finder, W3C COGA/STE100-inspired HCI manual, plain-language agent feedback, compact active-only mobile editing, immediate route/marker visibility beneath reduced-motion-safe continuity effects, computed 44px target and usable-map-area assertions, matched before/after Product Design QA, and current 1280x720/390x844 proof |
...
| Clean-clone and public-release safety audit | NEW LOCAL CANDIDATE VERIFIED / PRIVATE PUSH, DEPLOYMENT, PUBLICATION OWNER-GATED | Exact sanitized candidate `111b8fd068f08752cf76c194dd56f19778d95d2d`, projected from source `cc757316a7ff93d187536828c01e16671ff93b2f`, passed a no-local clean-clone frozen install, typecheck, build, 37/37 verifier, Chrome smoke with 9/9 official steps and 15 deeper executions, release audit, diff check, and clean-status proof. Private remote `main` and the active isolated Railway deployment deliberately remain on `986cf864927219f5f269a04b21a96f8440988cba` pending approval of this new exact SHA. |
