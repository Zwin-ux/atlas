...
| Live ChatGPT end-to-end evidence environment | PUBLIC CHROME AND REAL CODEX IN-APP VERIFIED / CHATGPT, MODEL GATED | Exact deployed candidate `39d1e141` is live at `https://atlas-webmcp-production.up.railway.app/explore` through isolated deployment `cd899386`; the newer private remote candidate is separately tracked below. HTTPS readiness, no-login, exact-five metadata, Chrome 152 and real Codex in-app Site Tools execution, visible atomic writes, Springfield safety, human marker-to-agent state handoff, refresh lifecycle, fallback, responsive Product Design proof, and reduced motion pass. The real ChatGPT transcript and exact `grok-4.6` three-run/90% lane remain separately gated. |
...
| Clean-clone and public-release safety audit | PRIVATE REMOTE VERIFIED / DEPLOYMENT AND PUBLICATION OWNER-GATED | Private remote `main` is exact presentation candidate `97792d84d66239745011624690769e99ab25838c`, projected from source `da0f4eae8582a345ada3de891422a21b0ac9c4dd`. It was pushed as a normal fast-forward from deployed `39d1e141`, remains private, and passes the complete no-local clean-clone sequence with 39/39 verification, exact-five Chrome proof, release audit, clean status, and a 16-commit Gitleaks scan with zero findings. Railway did not auto-deploy; the public route remains on independently verified candidate `39d1e141` through deployment `cd899386`. Deployment, public visibility, and submission remain separate owner gates. |
