...
| Live ChatGPT end-to-end evidence environment | PUBLIC CHROME VERIFIED / UPDATE OWNER-GATED / CHATGPT, MODEL GATED | Exact candidate `986cf864` remains live at `https://atlas-webmcp-production.up.railway.app/explore`; HTTPS readiness, exact-five metadata, Chrome 152 protocol execution, fallback, and responsive Product Design proof pass. Local candidate `39d1e141` adds the pinned current-source compatibility record, corrects registration lifecycle teardown, removes unsolicited idle coaching, strengthens immediate visible completion and mobile map/target proof, and hardens the one-command acceptance session against fabricated or uncontained evidence. The private push/deployment, real ChatGPT transcript, and exact `grok-4.6` three-run/90% lane remain separately gated. |
