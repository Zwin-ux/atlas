...
| National anchor truth | `data/census/us-county-town-anchors.json` covers all 3,222 supported counties with 13,797 real Census anchors; `verify:county-town-anchors` is release-gated. | 🚢 | Deploy the clean candidate, then test representative metro, frontier, island, and DC counties in ChatGPT. |
...
| Worker resilience | Local tests prove Redis `maxclients` retry, singleton adapter reuse, transient poll recovery, and worker readiness behavior. | 🚢 | Deploy the worker only with its Railway healthcheck configured, then verify live health/readiness. |
