...
const [toolsSource, registrySource, evalSchemaSource, mainSource, finderSource, appSource, plateSource, geometrySource, cssSource, serverSource, packageSource, modelEvalsSource, smokeEvalsSource, evalRunnerSource, chatgptSessionSource, chatgptRunnerSource, livePreflightSource, transcriptVerifierSource, transcriptTemplateSource, grokRunnerSource, railwayConfigSource, hciManualSource] = await Promise.all([
  ...
  readFile("scripts/prepare-chatgpt-session.mjs", "utf8"),
...
assert(cssSource.includes(".atlas-app__trail li:not(.is-active) .atlas-app__trail-prompt") && cssSource.includes(".atlas-app__trail-prompt-preview"), "Mobile trail editing must keep inactive stops compact without removing their context.");
assert(hciManualSource.includes("ASD-STE100") && hciManualSource.includes("does not claim ASD-STE100 conformance") && hciManualSource.includes("W3C Cognitive Accessibility") && hciManualSource.includes("prefers-reduced-motion"), "The public HCI manual must preserve the standards boundary and motion rule.");
...
assert(transcriptVerifierSource.includes("ambiguous_open_state") && transcriptVerifierSource.includes("failed_trail_state") && transcriptVerifierSource.includes("exactly match the prior visible workspace"), "Ambiguous and failed writes require independent post-failure state reads before ChatGPT evidence can pass.");
assert(transcriptVerifierSource.includes("dirname(transcriptPath)") && transcriptVerifierSource.includes("must stay inside the session evidence folder") && transcriptVerifierSource.includes("10_000_000") && transcriptVerifierSource.includes("PNG screenshot"), "ChatGPT evidence must stay inside the bounded session evidence folder and screenshots must be real PNG files.");
...
assert(JSON.stringify(transcriptTemplate.steps.map(({ id }) => id)) === JSON.stringify(["initial_state", "ambiguous_search", "open_resolved_place", "add_visible_note", "create_visible_trail", "manual_stop_state", "ambiguous_open_unchanged", "ambiguous_open_state", "failed_trail_unchanged", "failed_trail_state"]), "The ChatGPT transcript template must cover the complete shared-map acceptance journey.");
assert(typeof transcriptTemplate.evidence?.springfieldCandidatesScreenshot === "string", "ChatGPT evidence must capture the visible Springfield candidate state.");
